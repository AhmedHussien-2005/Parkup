...
export function renderMap(container) {
  container.innerHTML = `
    <div style="display:flex;flex-direction:column;height:100%;overflow:hidden">
      ...
      <!-- TOP BAR -->
      <div style="background:white;border-bottom:1px solid #E3F2FD;padding:12px 20px;display:flex;align-items:center;gap:12px;flex-shrink:0;flex-wrap:wrap;z-index:10">
        <h2 style="font-size:17px;font-weight:900;color:#0A2540;margin:0;white-space:nowrap">Live Parking Map</h2>

        <!-- Search -->
        <div style="position:relative;flex:1;min-width:180px;max-width:300px">
          <input id="map-search"
            style="width:100%;padding:8px 12px 8px 36px;border:1.5px solid #E3F2FD;border-radius:10px;font-size:13px;color:#0A2540;outline:none;font-family:'Inter',sans-serif;transition:border-color .2s"
            placeholder="Search a garage..."
            onfocus="this.style.borderColor='#29ABE2'"
            onblur="this.style.borderColor='#E3F2FD'"/>
        ...
        <div style="display:flex;gap:8px">
          <button id="chip-garages" onclick="toggleLayer('garages')"
            style="display:flex;align-items:center;gap:6px;padding:7px 13px;border-radius:20px;font-size:12px;font-weight:700;cursor:pointer;border:2px solid #29ABE2;background:#29ABE2;color:white;font-family:'Inter',sans-serif;transition:all .2s">
          ...
          <button id="chip-streets" onclick="toggleLayer('streets')"
            style="display:flex;align-items:center;gap:6px;padding:7px 13px;border-radius:20px;font-size:12px;font-weight:700;cursor:pointer;border:2px solid #1A73CC;background:#1A73CC;color:white;font-family:'Inter',sans-serif;transition:all .2s">
        ...
        <button onclick="openReport()"
          style="display:flex;align-items:center;gap:6px;padding:8px 16px;border-radius:10px;background:#29ABE2;color:white;font-size:13px;font-weight:700;border:none;cursor:pointer;font-family:'Inter',sans-serif;transition:background .2s;white-space:nowrap"
      ...
      <div style="flex:1;display:flex;min-height:0;overflow:hidden">
        ...
        <!-- Legend -->
        <div style="position:absolute;top:12px;right:12px;background:white;border-radius:12px;padding:10px 14px;box-shadow:0 4px 16px rgba(0,0,0,.12);border:1px solid #E3F2FD;z-index:500">
          <div style="font-size:10px;font-weight:700;color:#5B8DB8;letter-spacing:1px;text-transform:uppercase;margin-bottom:8px">Status</div>
          ${['available','few','full'].map(s=>`
            <div style="display:flex;align-items:center;gap:7px;margin-bottom:5px">
              <div style="width:11px;height:11px;border-radius:50%;background:${statusColor(s)};border:2px solid white;box-shadow:0 1px 4px rgba(0,0,0,.2)"></div>
              <span style="font-size:11px;font-weight:600;color:#0A2540">${statusLabel(s)}</span>
            </div>
          `).join('')}
          <div style="border-top:1px solid #E3F2FD;margin-top:6px;padding-top:6px;display:flex;align-items:center;gap:7px">
            <div style="width:11px;height:11px;border-radius:50%;background:#29ABE2;border:2px solid white;box-shadow:0 1px 4px rgba(0,0,0,.2)"></div>
      ...
      <!-- BOTTOM BAR -->
      <div style="background:#0A2540;padding:12px 20px;display:flex;align-items:center;gap:10px;flex-shrink:0">

        <!-- My Location -->
        <button onclick="goToMyLocation()"
          style="display:flex;align-items:center;gap:7px;padding:10px 14px;border-radius:10px;background:rgba(255,255,255,.1);color:white;font-size:13px;font-weight:700;border:1.5px solid rgba(255,255,255,.2);cursor:pointer;font-family:'Inter',sans-serif;transition:all .2s;white-space:nowrap"
        ...
        <!-- Report big -->
        <button onclick="openReport()"
          style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px;padding:11px;border-radius:10px;background:#29ABE2;color:white;font-size:14px;font-weight:700;border:none;cursor:pointer;font-family:'Inter',sans-serif;transition:background .2s"
          onmouseover="this.style.background='#1A73CC'" onmouseout="this.style.background='#29ABE2'">
          <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg>
          + Report an Empty Spot
        </button>
    ...
    <div style="display:flex;flex-direction:column;height:100%;overflow-y:auto;font-family:'Inter',sans-serif">
      ...
      <div style="padding:16px;display:flex;flex-direction:column;gap:12px;flex:1">
        ...
        <div style="background:#F8FBFF;border-radius:12px;padding:4px;border:1px solid #E3F2FD">
          ${[
            ['🕐', 'Working Hours', g.workingHours||'—'],
            ['💰', 'Price / Hour',  (g.pricePerHour||0)+' EGP'],
            ['🅿️', 'Available',     (g.availableSpots||0)+' / '+(g.totalSpots||0)+' spots'],
            ['📞', 'Manager Phone', g.managerPhone||'—'],
          ].map(([icon, label, val], i, arr) => `
            <div style="display:flex;align-items:center;gap:12px;padding:12px 12px;${i < arr.length-1 ? 'border-bottom:1px solid #EEF4FF' : ''}">
      ...
      <div style="padding:16px;display:flex;gap:10px;flex-shrink:0;border-top:1px solid #E3F2FD">
        <a href="https://www.google.com/maps/dir/?api=1&destination=${g.lat},${g.lng}" target="_blank"
          style="flex:1;display:flex;align-items:center;justify-content:center;gap:7px;padding:12px;background:#29ABE2;color:white;border-radius:10px;font-size:13px;font-weight:700;text-decoration:none;transition:background .2s"
          onmouseover="this.style.background='#1A73CC'" onmouseout="this.style.background='#29ABE2'">
          <svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><polygon points="3,11 22,2 13,21 11,13 3,11"/></svg>
          Get Directions
        </a>
        <a href="https://maps.google.com/?q=${g.lat},${g.lng}" target="_blank"
          style="padding:12px 16px;background:#0A2540;color:white;border-radius:10px;font-size:13px;font-weight:700;text-decoration:none;display:flex;align-items:center;gap:6px;transition:background .2s"
          onmouseover="this.style.background='#1255A0'" onmouseout="this.style.background='#0A2540'">
          <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/><circle cx="12" cy="9" r="2.5"/></svg>
          Maps
        </a>
      </div>
    </div>
